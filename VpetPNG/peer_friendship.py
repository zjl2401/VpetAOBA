...
ACTION_HOME_GUEST = "home_guest"  # 熟人起：邀请对方来家园做客

# 阶段制（光遇/火纹式）：level 1～5 ↔ stage 0～4；每级只解锁可感知特权
...
STAGE_DEFS: tuple[dict, ...] = (
    ...
    },
    {
        ...
        "name": "好友",
        "title": "信得过的人",
        "perms": ("组队小加成（预告）", "借用低阶资源（预告）"),
        "unlock_action": "",
        "unlock_label": "共同订单 / 组队支援",
        "bonus_hint": "并肩时小加成（即将开放）",
    },
    {
        "stage": 3,
        "level": 4,
        "name": "挚友",
        "title": "并肩的人",
        "perms": ("专属双人动作（预告）", "共享家具蓝图（预告）"),
        "unlock_action": "",
        "unlock_label": "击掌 / 拥抱等亲密动作",
        "bonus_hint": "需双方确认（即将开放）",
    },
    {
        "stage": 4,
        "level": 5,
...
# 每日友情经验上限（防刷）；同质行为递减；多通道有 bonus
DAILY_XP_CAP = 100.0
CHANNEL_DIMINISH = 0.55  # 同日同通道第 2 次起乘数
CHANNEL_DIMINISH_FLOOR = 0.2
DIVERSITY_BONUS = 0.15  # 当日已有 ≥2 种通道时额外加成
GAIN_CHANNELS = frozenset({"meet", "talk", "quiz", "gift", "help", "focus", "generic"})
...
def daily_xp_state(data: dict, *, today: str | None = None) -> dict:
    ...
    if str(data.get("daily_xp_ymd") or "") != day:
        return {
            "ymd": day,
            "xp": 0.0,
            "left": DAILY_XP_CAP,
    ...
    xp = max(0.0, float(data.get("daily_xp") or 0))
    ch = data.get("daily_channels") or {}
...
def relation_card(presence_dir: Path) -> dict:
    ...
    maintain = today_maintain_tips(data, st)
    return {
        **st,
...
def today_maintain_tips(data: dict, st: dict | None = None) -> list[str]:
    """今日可维护：1～2 条低成本建议（P5 轻量版）。"""
    tips: list[str] = []
    daily = daily_xp_state(data)
    if daily["capped"]:
        return ["今日友情经验已满，聊两句也很好，不必再肝"]
    st = st or stats(float(data.get("points") or 0))
    channels = daily.get("channels") or {}
    n = int(data.get("meet_count") or 0)
    if n < 11:
        tips.append("再一起点几次「遇」，熟悉彼此")
    elif "meet" not in channels:
        tips.append("靠近对方点一次「遇」或「话」")
    if "quiz" not in channels and n >= 11:
        tips.append("聊一聊问答，答对喜好记得加分")
    if int(st.get("level") or 1) < HOME_GUEST_UNLOCK_LEVEL:
        tips.append(f"升到「熟人」可邀请对方来家园做客")
    elif "help" not in channels:
        tips.append("进家园邀请做客，也算一次维护（异步帮忙即将开放）")
    # 去重保 2 条
    out: list[str] = []
    for t in tips:
        if t not in out:
    ...
    try:
        ...
        if isinstance(raw, dict):
            ...
            return {
                ...
                "daily_xp_ymd": str(raw.get("daily_xp_ymd") or ""),
                "daily_xp": max(0.0, float(raw.get("daily_xp") or 0)),
                "daily_channels": {str(k): int(v) for k, v in ch.items()},
                "milestones": [str(x) for x in milestones if str(x).strip()][-16:],
            }
    except Exception:
        pass
    return dict(empty)
...
def save(presence_dir: Path, data: dict) -> None:
    try:
        ...
        payload = {
            ...
            "daily_xp_ymd": str(data.get("daily_xp_ymd") or ""),
            "daily_xp": max(0.0, float(data.get("daily_xp") or 0)),
            "daily_channels": {str(k): int(v) for k, v in ch.items()},
            "milestones": [str(x) for x in milestones if str(x).strip()][-16:],
        }
...
def record_milestone(presence_dir: Path, event_id: str) -> bool:
    """写入关键事件槽（去重）；供 P2 相册用，P0 先落盘。"""
    eid = str(event_id or "").strip()
    if not eid:
        return False
    data = load(presence_dir)
    ms = [str(x) for x in (data.get("milestones") or [])]
    if eid in ms:
        return False
    ms.append(eid)
    data["milestones"] = ms[-16:]
...
def apply_xp_gain(
    ...
    *,
    ...
    """应用每日上限 + 同质递减 + 多样 bonus。返回 (实际加点, 元信息)。"""
    ...
    used = max(0.0, float(data.get("daily_xp") or 0))
...
def gain_friendship_xp(
    ...
    *,
    ...
    """加点并回写；带日上限。正数走通道规则，负数直接扣。"""
    data = load(presence_dir)
    before = stats(float(data.get("points") or 0))
    applied, meta = apply_xp_gain(data, float(delta), channel=channel)
    save(presence_dir, data)
    after = stats(float(data.get("points") or 0))
    leveled = int(after.get("level") or 1) > int(before.get("level") or 1)
    if leveled:
        record_milestone(presence_dir, f"stage_{after.get('stage_name')}")
    daily = daily_xp_state(data)
    return {
        **data,
        **after,
...
def record_meet(
    ...
    *,
    ...
    before_lv = int(stats(float(data.get("points") or 0)).get("level") or 1)
    apply_xp_gain(data, gain, channel="meet")
    data["last_meet_ms"] = int(now_ms)
    n = int(data["meet_count"])
    if n == 1:
        ms = [str(x) for x in (data.get("milestones") or [])]
        if "first_meet" not in ms:
            ms.append("first_meet")
            data["milestones"] = ms[-16:]
    if n == 11:
        ms = [str(x) for x in (data.get("milestones") or [])]
        if "first_intro" not in ms:
            ms.append("first_intro")
            data["milestones"] = ms[-16:]
    save(presence_dir, data)
    st = stats(float(data["points"]))
    if int(st.get("level") or 1) > before_lv:
        record_milestone(presence_dir, f"stage_{st.get('stage_name')}")
        data = load(presence_dir)
        st = stats(float(data.get("points") or 0))
    phase = meet_phase(n)
    daily = daily_xp_state(data)
...
def next_unlock_hint(level: int) -> str | None:
    st = _enrich_stage_fields(
        {
            "level": max(1, int(level)),
            "level_capped": int(level) >= MAX_IMPLEMENTED_LEVEL,
        }
    )
    return str(st.get("next_unlock") or "后续阶段暂未开放")


def can_invite_home_guest(level: int) -> bool:
    return int(level) >= int(HOME_GUEST_UNLOCK_LEVEL)


def permission_unlocked(level: int, action: str) -> bool:
    act = str(action or "")
    if act == ACTION_STROLL_TOGETHER:
        return int(level) >= 1
    if act == ACTION_HOME_GUEST:
        return can_invite_home_guest(level)
    req = None
    for lv, a in ACTION_BY_LEVEL.items():
        if a == act:
            req = int(lv)
            break
    if req is None:
        return False
    return int(level) >= req
...
def add_friendship_points(presence_dir: Path, delta: float, *, channel: str = "generic") -> dict:
    """加减友情点数并回写（正数走日上限/递减；负数直接扣）。"""
    return gain_friendship_xp(presence_dir, float(delta), channel=channel)

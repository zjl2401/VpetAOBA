...
| border2 对话条、面板 border 九宫格 | ✅ 必须 | UI 增强，非语音 |
| 出场逆向像素扫描 | ✅ 必须 | 关闭桌宠时播放 |
| 语音播放、border2 语音字幕、Vpetvoice 扫描 | ❌ 不在此表 | 见 `voice_system.py`，仅作附加层 |
...
| D03 | border2 对话条 | 九宫格横条，去白边，随内容伸缩 | `_layout_speech_dialog` |
| D04 | 无台词动作 | wink、脸红、有主意、下蹲无 banter | `_interact_flair` banter 配置 |
| D05 | ×生活文案 | 固定 ADULT 文案 | `_play_adult_msg` |
| D06 | 打电话 | **打字框 CALL_TEXT + 原铃声** | `_show_call_dialog` / `_play_call_audio` |
| D07 | 打招呼 | HI 文本 + 打字 | `_play_action("hi")` |

**语音附加规则**：字幕走 `_show_voice_subtitle` 独立窗口，**不得**调用 `_show_speech_dialog` 替代上述对话框。
...
| # | 功能 | 预期行为 | 代码锚点 |
|---|------|----------|----------|
| P01 | 面板 border | `border.png` 九宫格随面板伸缩 | `_layout_panel_border` |
| P02 | 体力/心情/背包 | 数值展示与刷新 | `_refresh_panel` |
| P03 | 食物拖拽喂食 | 选食物拖给小人，恢复体力心情 | `_feed_food` / `_play_eat_food` |
| P04 | 吃东西动画 | eat 序列 + 食物 FX + 咀嚼音 | `_play_eat_food` / `_play_eat_sound` |
| P05 | 智能伴侣金目 | 100px 侧向跟随；游戏跟紧；工作导航 | `_mini_pet_follow_tick` |
| P06 | 面板弹出定位 | 靠屏外方向展开 | `_place_panel_popup` |
...
2. `voice_mode` 开启时：原功能照常 + `_addon_voice_vpet` / `_show_voice_subtitle` 并行。
...
> **操作要求**：复制本表到新 commit / 打包前的自检记录，逐项打勾 `[x]`。任一项未实现则不得发布。
...
### 对话与特效
- [ ] D01–D07 打字框与 border2 正常；打电话有打字+原铃
- [ ] F01–F11 各表情特效可见且会清理
- [ ] 开语音时侧踢/伤心/喂食仍有粒子与台词
...
### 面板与动画
- [ ] P01–P06 面板 border、喂食、金目正常
- [ ] A01–A04 出场逆向；退出时语音播完再消失

### 语音隔离
- [ ] 关语音时与上表完全一致
- [ ] 开语音时原功能未被 `return` 跳过
- [ ] 语音字幕独立窗口，未抢占 `speech_dialog`
...
**最后更新**：2026-07-13（语音附加层与出场逆向扫描纳入验收）

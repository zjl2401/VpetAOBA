...
PET_ID_FEATURE = True
# 录实况 / 演示：True=每次启动都弹出操作说明；正式发布前改 False（仅首次弹一次）
DEMO_ALWAYS_SHOW_GUIDES = True
...
COMMUNITY_FOLLOW_BLURB = (
    ...
    "本桌宠免费开放，为《戏剧性谋杀》同人二创。感谢游玩，祝游玩愉快 (´∀｀)♡"
)
...
class DesktopPet:
    ...
    def _open_operation_guide(self, *, auto: bool = False) -> None:
        ...
        _pack_community_follow_blurb(frame, wraplength=GUIDE_TEXT_WRAP, pady=(0, 6))
    ...
    def _begin_rhyme_fight(self) -> None:
        ...
        def end_fight(won: bool) -> None:
            ...
            if won:
                ...
                refresh()
                self._play_game_fail_voice("hurt")
                # G08：保留对战界面约 1.6s 后关闭（非全屏 game_clear）
    ...
    def _open_about(self) -> None:
        ...
        _pack_community_follow_blurb(frame, wraplength=300, pady=(0, 6))
        for label, url in COMMUNITY_LINKS:
            _pack_web_link(frame, label, url, prefix="· ")

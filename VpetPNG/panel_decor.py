...
# 上白下蓝（底端略加深，右键菜单/面板层次更清晰）
THEME_BG_TOP = "#ffffff"
THEME_BG_MID = "#c8e6ff"
THEME_BG_BOTTOM = "#6eb0d8"
# 蓝系面板底色（用于决定是否上浅蓝渐变）
...
def apply_transparent_text_plates(root, *, alpha: float | None = None) -> None:
    ...
    def _match_plate(w, cls: str) -> None:
        ...
        if bg_l in _KEEP_PLATE_BGS:
            return
        should = cls in ("Label", "Message", "Checkbutton", "Radiobutton", "Frame", "Labelframe", "Text") or is_text_plate_bg(
            bg
        )
        ...
        try:
            ...
            wh = max(int(w.winfo_height()), 1)
            # 高块勿取几何中心，否则整段染成中段色，看起来像嵌套色框
            sample_off = 2 if wh > 22 else wh // 2
            mid_y = int(w.winfo_rooty()) - host_top + sample_off
            ...
            for opt in ("highlightbackground", "activebackground", "selectcolor", "insertbackground"):
                try:
    ...
    def _walk(w) -> None:
        ...
        if cls == "Canvas":
            try:
                ...
                pass
        _match_plate(w, cls)
        if cls in ("Label", "Message", "Checkbutton", "Radiobutton", "Text"):
            _deepen_widget_fg(w)
        try:
...
def pack_panel_shell(
    ...
    *,
    ...
    inner = tk.Frame(body, bg=THEME_BG_TOP if do_grad else body_bg, padx=padx, pady=pady)
    if do_grad:

...
def main() -> None:
    ...
    # 路径优先：源码 > 安装根。
    # 切勿把 _internal 放进 sys.path：里面有打包的 numpy，会盖住本机包，
    # pygame.sndarray / 音乐试听等会坏，表现为「快捷方式开了但功能没改」。
    for p in (here, src):
        if p is None:
            continue
        s = str(p)
        while s in sys.path:
            sys.path.remove(s)
        sys.path.insert(0, s)
    # 防御：即使别处把 _internal 塞进 path，也挪到末尾
    if internal.is_dir():
        s_int = str(internal)
        while s_int in sys.path:
            sys.path.remove(s_int)

    # 立绘/资源走 _MEIPASS；不要改 sys.executable，否则 tkinter 找不到 Tcl

...
def main() -> int:
    # 若已有独立黑框源（VpetAOBAblack 同步过），勿用当前 sprites 覆盖 sprites_black
    desktop_black = Path.home() / "Desktop" / "VpetDMMd" / "VpetAOBAblack"
    has_dedicated_black = desktop_black.is_dir() and any(
        p.is_file() and p.suffix.lower() in {".jpg", ".jpeg", ".png"} for p in desktop_black.iterdir()
    )
    if has_dedicated_black:
        print(f"检测到独立黑框源 {desktop_black}，跳过把当前 sprites 备份进 sprites_black")
        print("（请用 tools_sync_aoba_black.py 维护黑框图组）")
    else:
    ...
    # 道具已在 props，勿把黑框 box/flag 再填进 sprites
    filled_s = [n for n in filled_s if n.lower() not in PROP_NAMES]
